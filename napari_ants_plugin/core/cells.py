...
def remove_duplicate_cells(cell_locations, cell_size_radius=5):
    """
    ...
    try:
        cell_locations_np = np.array(cell_locations, dtype=np.float64) # Use float64 for precision
        if cell_locations_np.ndim == 1 and len(cell_locations_np) == 3: # Handle single point input as list/tuple
            cell_locations_np = cell_locations_np.reshape(1, -1) # Reshape to 2D array
        elif cell_locations_np.ndim != 2 or cell_locations_np.shape[1] != 3:
            raise ValueError("cell_locations must be a list of tuples/lists/numpy arrays of length 3 (z, y, x).")
    ...
    if cell_locations_np.shape[0] <= 1: # Optimization: No duplicates possible with 0 or 1 point
        return [tuple(loc) for loc in cell_locations_np.tolist()] # Return as tuples, consistent output
    ...
    for i in range(cell_locations_np.shape[0]):
        ...
        nearby_indices = kdtree.query_ball_point(cell_locations_np[i], r=cell_size_radius)

        # Add the current cell location as it's the representative of this cluster
        unique_cells.append(tuple(cell_locations_np[i].tolist())) # Convert back to tuple for list of tuples output
        ...
        with open(csv_filepath, 'r') as csvfile:
            ...
            for row in csv_reader:
                try:
                    ...
                    print(f"Warning: Skipping invalid row in CSV: {row}.  Expected numerical (z, y, x) values.")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Remove duplicate cell locations based on cell size radius.')

    print(remove_duplicate_cells.__doc__) # Print function docstring

    parser.add_argument('--input_csv', '-i', type=str, help='Path to the input CSV file containing cell locations (z, y, x) per row.')
    parser.add_argument('--radius', '-r', type=float, default=5.0, help='Cell size radius for considering duplicates (default: 5.0).') # Default radius is now 5.0
    parser.add_argument('--output_csv', '-o', type=str, help='Path to save the unique cell locations to a CSV file.')
    ...
    if args.input_csv:
        try:
            ...
            if not cell_locations:
                ...
                print(f"Loaded {len(cell_locations)} locations from CSV: {args.input_csv}")
        except FileNotFoundError as e:
            print(f"Error: {e}")
            exit(1) # Exit with error code
    else:
        # Default locations if no CSV provided for demonstration
        cell_locations = [(1, 2, 3), (1.1, 2.1, 3.2), (4, 5, 6), (1, 2, 3.1), (7, 8, 9)]
    ...
    if cell_locations: # Only process if there are locations to process
        unique_locations = remove_duplicate_cells(cell_locations, cell_size_radius)
        ...
        if args.output_csv:
            ...
            try:
                with open(output_csv_filepath, 'w', newline='') as csvfile:
                    ...
                    csv_writer.writerows(unique_locations) # Write list of tuples directly
            ...
            print(f"Unique Locations (first 10): {unique_locations[:min(10, len(unique_locations))]}")
            print("To save unique locations to a CSV file, use the --output_csv or -o argument.")
    ...
    if not args.input_csv:
        ...
        locations1 = [(1, 2, 3), (1.1, 2.1, 3.2), (4, 5, 6), (1, 2, 3.1), (7, 8, 9)]
        radius1 = 0.5
        ...
        print(f"Test 2 - Locations: {locations2}, Radius: {radius2}, Unique Locations: {unique_locations2}")

        locations3 = [(0, 0, 0), (0, 0, 0), (0, 0, 0)] # All duplicates
        radius3 = 0.01
        unique_locations3 = remove_duplicate_cells(locations3, radius3)
        print(f"Test 3 - Locations: {locations3}, Radius: {radius3}, Unique Locations: {unique_locations3}")

        locations4 = [(10, 20, 30)] # Single location
        radius4 = 1.0
        unique_locations4 = remove_duplicate_cells(locations4, radius4)
        print(f"Test 4 - Locations: {locations4}, Radius: {radius4}, Unique Locations: {unique_locations4}")

        locations5 = [(2, 2, 2), (5, 5, 5), (2.05, 2.05, 2.05), (8, 8, 8), (5.1, 5.1, 5.1)]
        radius5 = 0.2
        unique_locations5 = remove_duplicate_cells(locations5, radius5)
        print(f"Test 5 - Locations: {locations5}, Radius: {radius5}, Unique Locations: {unique_locations5}")