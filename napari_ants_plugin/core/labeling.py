...
def generate_countgd_labels(image_roi, label_type='points', text_prompt=DEFAULT_TEXT_PROMPT, exemplar_image=None, exemplar_points=None):
    """
    ...
    if is_color_2d:
        ...
        image_pil = Image.fromarray(
            image_roi_np.astype(np.uint8), 'L').convert('RGB')
    ...
    if exemplar_image is not None:
        if isinstance(exemplar_image, np.ndarray):
            ...
            exemplar_image_pil = Image.fromarray(exemplar_image_np.astype(
                np.uint8), 'RGB')  # Convert numpy exemplar to PIL
